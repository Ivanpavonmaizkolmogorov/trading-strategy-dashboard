...
class MyfxbookClient:
    ...
    def get_history(self, account_id: int) -> List[Dict]:
        ...
        # Fetching from 2010 to ensure we capture ALL trades
        # Myfxbook requires start date for full history, otherwise defaults might apply
        ...
        try:
            print(f"[Myfxbook] 🔄 Fetching FULL history (2010 to {end_date}) from: {url}")
            ...
            # Print precise URL for debugging (excluding session for safety)
            req = requests.Request('GET', url, params=params)
            prepped = req.prepare()
            print(f"[Myfxbook] 🔗 Requested URL (masked): {prepped.url.rsplit('session', 1)[0]}session=***")
            ...
            data = response.json()
            print(f"[Myfxbook] 📦 Full response data (History): {str(data)}")
            print(f"[Myfxbook] 🔢 Raw 'history' list length: {len(data.get('history', []))}")
