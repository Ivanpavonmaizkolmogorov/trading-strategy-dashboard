...
const AVAILABLE_COLUMNS = [
    ...
    { id: 'mt5ConnectionDate', label: 'MT5 Connect Date', minWidth: 140 }
];
...
const renderAdvancedFilterPanel = () => {
    ...
    if (!state.advancedFilters) {
        state.advancedFilters = { searchText: '', mt5Only: false, linkedOnly: false, selectedPortfolioId: 'all', showQuarantined: false, mt5MinMonths: 0 };
    }
    ...
    panel.innerHTML = `
        <div class="flex items-center gap-4 flex-1">
            ...
            <!-- MT5 Connection Date Filter - Chip Buttons -->
            <div class="flex items-center gap-1 bg-gray-900/50 p-1 px-2 rounded-lg border border-gray-700/50" title="Filtrar por antigüedad de enchufe a MT5">
                <span class="text-[10px] text-gray-500 font-bold whitespace-nowrap mr-1">⚡ Enchufada:</span>
                <button class="mt5-age-chip px-2 py-0.5 rounded text-[10px] font-bold transition-all ${(!state.advancedFilters?.mt5MinMonths) ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}" data-months="0">Todas</button>
                <button class="mt5-age-chip px-2 py-0.5 rounded text-[10px] font-bold transition-all ${(state.advancedFilters?.mt5MinMonths === 3) ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}" data-months="3">> 3m</button>
                <button class="mt5-age-chip px-2 py-0.5 rounded text-[10px] font-bold transition-all ${(state.advancedFilters?.mt5MinMonths === 6) ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}" data-months="6">> 6m</button>
                <button class="mt5-age-chip px-2 py-0.5 rounded text-[10px] font-bold transition-all ${(state.advancedFilters?.mt5MinMonths === 12) ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}" data-months="12">> 1 año</button>
            </div>
        ...
        <div class="flex items-center gap-3 border-l border-gray-700 pl-4">
             ...
             <button id="adv-filter-date-all" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border bg-gray-700 border-gray-600 text-gray-400 hover:text-blue-300 hover:border-blue-500 transition-all" title="Aplicar filtro de fecha a TODAS las estrategias">
                 <span class="text-sm">📅</span><span class="text-xs font-bold">Filtrar Histórico</span>
             </button>
    ...
    // [NEW] 3.5 MT5 Enchufada Age Chip Buttons
    document.querySelectorAll('.mt5-age-chip').forEach(chip => {
        chip.addEventListener('click', () => {
            const months = parseInt(chip.dataset.months);
            if (!state.advancedFilters) state.advancedFilters = {};
            state.advancedFilters.mt5MinMonths = months || 0;
            renderStrategiesTable();
        });
    });
...
export const getMetricValue = (strategy, metricKey) => {
    ...
    if (state.activeViewMode === 'reality-check' && strategy.realMetrics) {
        if (strategy.realMetrics[metricKey] !== undefined) {
            val = strategy.realMetrics[metricKey];
        }
    }

    // [NEW] Provide structured formatting for connection date
    if (metricKey === 'mt5ConnectionDate') {
        const checkVal = strategy.realMetrics?.mt5ConnectionDate || source['mt5ConnectionDate'] || strategy['mt5ConnectionDate'];
        if (checkVal) {
            // Avoid double formatting
            if (typeof checkVal === 'string' && checkVal.includes(' ')) return checkVal;
...
export const renderStrategiesTable = () => {
    try {
        ...
        if (!tableBody || !tableHead) { console.warn('[StrategiesTable] ⚠️ DOM elements not found, aborting render.'); return; }
    ...
    `;
        ...
        if (state.activeViewMode === 'reality-check' && state.savedPortfolios && state.savedPortfolios.length > 0) {
            ...
            state.savedPortfolios.forEach((p, pIdx) => {
                ...
                const processStrategy = (name) => {
                    ...
                    if (p.realMetrics && p.realMetrics._tradesById) {
                        ...
                        const mapEntry = mapById || mapByName || mapByClean || mapByNorm;
                        ...
                        if (mapEntry) {
                            ...
                            mappedKeys.forEach(k => {
                                ...
                                if (p.linkedAccountId) {
                                    ...
                                    if (p.realMetrics._tradesById && p.realMetrics._tradesById[compoundKey]) {
                                ...
                                if (!foundTrades && p.realMetrics._tradesById && p.realMetrics._tradesById[keyStr]) {
                                    foundTrades = p.realMetrics._tradesById[keyStr];
                                    usedKey = keyStr;
                                    // console.log(`[StrategiesTable] 🎯 Found trades via Raw Key: ${keyStr}`);
                                }
                        ...
                        // Dedup Trades locally before calculating metrics
                        if (trades.length > 0) {
                            const seen = new Set();
                            // 1. DEDUP TRADES BY TICKET IF AVAILABLE
                            trades = trades.filter(t => {
                                const tid = t.ticket ? t.ticket : (t.openTime + t.symbol);
                                if (seen.has(tid)) return false;
                                seen.add(tid);
                                return true;
                            });

                            // [NEW] Track MT5 connection date while analyzing combined real trades
                            ...
                            trades.forEach(t => {
                                ...
                                if (!isNaN(tDate.getTime())) {
                                    ...
                                    if (!mt5ConnectionDate || isoDate < mt5ConnectionDate) {
                                        mt5ConnectionDate = isoDate;
                                    }
                                }
                            });

                            if (mt5ConnectionDate) {
                                if (!stratObj.realMetrics) stratObj.realMetrics = {};
                                stratObj.realMetrics.mt5ConnectionDate = mt5ConnectionDate;
                            }
                    ...
                    if (trades.length > 0) {
                        ...
                        } // End if (engineMetrics)
                    } // End if (trades.length > 0)

                    if (stratObj) {
                        strategies.push(stratObj);
                    }
                }; // End processStrategy function
            ...
            }); // End state.savedPortfolios.forEach
        ...
        // [PERF] Reduced logging — was 4 calls per render
        // console.log(`[StrategiesTable] 🛡️ Applying Filter: ${state.linkedStrategiesFilter.toUpperCase()} | Total: ${strategies.length} | Maps: ${linkedStrategiesMap.size}/${linkedStrategyNamesMap.size}`);
        ...
        const { searchText, mt5Only, magicMissingOnly, showQuarantined, selectedPortfolioId = 'all' } = advancedFilters;
        ...
        strategies = strategies.filter(s => {
            ...
            if (magicMissingOnly) {
                ...
                // [PERF] Filter debug log silenced
            ...
            // [NEW] 3.6 MT5 Enchufada Age Filter (Chip Buttons)
            if (advancedFilters?.mt5MinMonths && advancedFilters.mt5MinMonths > 0) {
                // Calculate the cutoff date: must have been plugged in BEFORE this date
                const cutoffDate = new Date();
                cutoffDate.setMonth(cutoffDate.getMonth() - advancedFilters.mt5MinMonths);
                const cutoffTs = cutoffDate.getTime();

                // Get plug-in date for this strategy (same logic as badge)
                let plugInTs = null;
                if (state.magicNumberMap && state.deepScanData) {
                    const keysForMagic = [
                        s.id, s.name,
                        normalizeName(s.name || ''),
                        String(s.name || '').replace(/\.csv$/i, '').trim()
                    ];
                    let magics = [];
                    ...
                    magics.forEach(magic => {
                        ...
                        if (key.includes('::')) {
                            const [accId, magicNum] = key.split('::');
                            const acc = state.deepScanData[accId];
                            if (acc) {
                                const tMap = acc.tradesById || acc._tradesById;
                                const trades = tMap ? tMap[magicNum] : null;
                                if (trades) trades.forEach(t => {
                                    const dv = t.openTime || t.entry_date || t.date;
                                    if (dv) { const ts = new Date(dv).getTime(); if (!isNaN(ts) && ts < earliest) earliest = ts; }
                                });
                            }
                        } else {
                            Object.values(state.deepScanData).forEach(acc => {
                                const tMap = acc.tradesById || acc._tradesById;
                                if (!tMap) return;
                    ...
                    if (earliest !== Infinity) plugInTs = earliest;
                }

                // No plug-in date found → exclude strategy (has no MT5 data)
                if (!plugInTs) return false;
                // Plug-in date must be BEFORE the cutoff (i.e., been running long enough)
                if (plugInTs > cutoffTs) return false;
            }
            ...
            if (state.linkedStrategiesFilter === 'hide') {
                ...
                if (isLinked) return false;
            } else if (state.linkedStrategiesFilter === 'only' || (selectedPortfolioId === 'all' && advancedFilters.linkedOnly)) { // Fallback for header toggle
                const norm = normalizeName(s.name);
                const isLinked = linkedStrategiesMap.has(s.originalIndex) ||
                    linkedStrategyNamesMap.has(s.name) ||
                    linkedStrategyNormalizedNamesMap.has(norm);
        ...
        if (state.activeViewMode === 'reality-check' && !sourcedFromPortfolios) {
            // [PERF] Reality check filter debug block silenced (was 10+ log calls)
            // Only filter applied is Zero Trades Cleanup below.


            // FILTER: Reality Check Mode - Zero Trades Cleanup
            // User Request: "habria k filtrar akellas k trades sea 0 (en este modo)"
            if (state.activeViewMode === 'reality-check') {
                // [PERF] Zero trades filter — logging silenced
        ...
        strategies.forEach((strategy, index) => {
            ...
            const stratName = strategy.name || strategy.id || '';
            const cleanStratName = stratName.replace(/\.csv$/i, '').trim();

            let originalIndex = -1;
            if (state.loadedStrategyFiles && Array.isArray(state.loadedStrategyFiles)) {
                originalIndex = state.loadedStrategyFiles.findIndex(f => {
                    if (!f || !f.name) return false;
                    return f.name === stratName || f.name.replace(/\.csv$/i, '').trim() === cleanStratName;
                });
            } else {
                originalIndex = strategy.originalIndex ?? -1;
            }

            // [PERF] Per-row log silenced (was 87 calls per render)
        ...
        `;
            ...
            config.visibleColumns.forEach(colId => {
                ...
                `;
                    ...
                    // DEBUG BADGE:
                    // [PERF] Badge check diagnostics silenced (was per-row for first 5)
                    ...
                    // [PERF] Magic number map diagnostic silenced (was logged on first row)
                    ...
                    if (state.magicNumberMap) {
                        ...
                        // [PERF] Per-strategy magic check diagnostic silenced
                    }
                    ...
                    if (hasMagicNumber && state.magicNumberMap && state.deepScanData) {
                        ...
                        if (earliestDate !== Infinity) {
                            ...
                            plugInDateStr = `${day} ${month} ${year}`;
                        }
                    }

                    const mt5Title = plugInDateStr
                        ? `Enchufada desde: ${plugInDateStr}`
                        : `Magic Number asignado (sin trades reales aún)`;

                    if (hasMagicNumber) {
                        // APPEND MT5 BADGE (with plug-in date if available)
                        const mt5Label = plugInDateStr ? `⚡ MT5 · ${plugInDateStr}` : '⚡ MT5';
                        html += `
                            <div class="inline-flex items-center ml-2 px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-indigo-900 text-indigo-200 border border-indigo-600 cursor-help whitespace-nowrap" title="${mt5Title}">
                                ${mt5Label}
                            </div>
                        `;
                    }
                ...
                row.appendChild(td);
            }); // End of config.visibleColumns.forEach
        ...
        // Update floating action bar visibility (deferred to avoid const hoisting issue)
        setTimeout(() => { if (typeof updateFloatingActionBar === 'function') updateFloatingActionBar(); }, 0);
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 w-96 p-6 shadow-2xl transform scale-100 transition-transform">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                <button class="sd-quick-chip px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-[10px] font-bold transition-colors" data-days="30">Últimos 30 días</button>
                <button class="sd-quick-chip px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-[10px] font-bold transition-colors" data-days="90">Últimos 3 meses</button>
                <button class="sd-quick-chip px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-[10px] font-bold transition-colors" data-days="180">Últimos 6 meses</button>
                <button class="sd-quick-chip px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-[10px] font-bold transition-colors" data-days="365">Último año</button>
                <button class="sd-quick-chip px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded text-[10px] font-bold transition-colors" data-ytd="true">YTD</button>
            </div>
    ...
    `;
            ...
            const fpStart = flatpickr("#sd-start", fpConfig);
            const fpEnd = flatpickr("#sd-end", { ...fpConfig, defaultDate: currentRange.end || 'today' });

            // [NEW] Quick Chip Logic
            document.querySelectorAll('.sd-quick-chip').forEach(btn => {
            ...
            document.getElementById('sd-save').onclick = () => {
                ...
                close(); // [FIX] Added close() here so the modal actually closes when you click apply
                renderStrategiesTable();
    ...
    `;
        ...
        console.error('[StrategiesTable] 🔴🔴🔴 CRITICAL ERROR in renderStrategiesTable:', err);
        console.error('[StrategiesTable] Stack:', err.stack);
    }
};
